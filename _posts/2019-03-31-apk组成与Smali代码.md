...
apktool这个工具在反编译apk后会得到大量的Smali格式的文件，这些文件就是我们这个项目中除了资源文件外的所有代码所生成的底层指令，所以其中包括项目中implementation库、jar包、aar包等。Smali其实是一门很简单的汇编语言，是用于Dalvik虚拟机执行的指令与高级语言中间的代码，在日常反编译中我们得到的java代码都是工具根据smali得到的，并不是真正的源码，大部分可用，但也会出现反编译出错，或者代码可读性差等错误，所以Smali语言在反编译之路上还是很值得学习的。

学习方法上，我主要是通过对照java代码学习smali语法。

### 数据的表示

 I=>int		Z=>boolean		B=>byte		S=>short		V=>void

C=>char 	L=>long			F=>float  	D=>double

L完整类名==对象类型 

例如：	Ljava/lang/String Landroid/content/Context
[==数组类型 
...
一个activity中如果有多个实例并且有回调，在编译时就会生成多个activity\$1、activity\$2、activity$3等带有\$符号的文件,代表匿名内部类以及内部类等。
