...
Smali其实是一门很简单的汇编语言，是用于Dalvik虚拟机执行的指令与高级语言中间的代码，在日常反编译中我们得到的java代码都是工具根据smali得到的，并不是真正的源码，大部分可用，但也会出现反编译出错，或者代码可读性差等错误，所以Smali语言在反编译之路上还是很值得学习的。
...
带有-object代表是对象类型，不带则是基本类型，boolean类型例外
